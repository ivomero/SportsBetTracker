  ...
  const getGames = async () => {
    try {
      let response = await axios.get(
        " https://api.the-odds-api.com/v4/sports/americanfootball_nfl/odds?regions=us&oddsFormat=american&apiKey=ac2e471edb79cd7c4357c66c9a625f11"
      );
    ...
    <Fragment>
      <div className="GamesList">
        ...
        {games.map((game) => (
          <div className="Games">
            <h2 className="away">
              {game.bookmakers[4].markets[0].outcomes[0].name}
            </h2>
            <h2 className="away">
              {game.bookmakers[4].markets[0].outcomes[0].price}
            </h2>
            <button
              className="button"
              onClick={() =>
                props.postBet(game, game.away_team, props.currentUser)
              }
            >
              Bet Unit
            </button>
            <h2 className="home">
              {game.bookmakers[4].markets[0].outcomes[1].name}
            </h2>
            <h2 className="home">
              {game.bookmakers[4].markets[0].outcomes[1].price}
            </h2>
            <h4>{game.commence_time}</h4>
            <button
              className="button"
              onClick={() =>
                props.postBet(game, game.home_team, props.currentUser)
              }
            >
              Bet Unit
            </button>
            <form
              onSubmit={(e) => handleCommentSubmit(e, props.currentUser, game)}
            >
              <input
                type="text"
            ...
            <Link to={`/game-comments/${game.id}`}>View Comments</Link>
